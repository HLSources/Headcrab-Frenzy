...
extern CMenuPanel *CMessageWindowPanel_Create( int server, const char *szMOTD, const char *szTitle, int iShadeFullscreen, int iRemoveMe, int x, int y, int wide, int tall );
...
TeamFortressViewport::TeamFortressViewport(int x,int y,int wide,int tall) : Panel(x,y,wide,tall), m_SchemeManager(wide,tall)
{
	...
	m_SpectatorPollsMenu = CreateCommandMenu("spectvotemenu.txt", 1, YRES(32), true, CMENU_SIZE_X, BUTTON_SIZE_Y / 2, XRES(93+30) );	// above bottom bar, flat design
	CreateServerBrowser();
}
...
void TeamFortressViewport::HideCommandMenu()
{
	...
	if ( m_pCommandMenus[m_SpectatorPollsMenu] )
	{
		m_pCommandMenus[m_SpectatorPollsMenu]->ClearButtonsOfArmedState();
	}
...
CMenuPanel* TeamFortressViewport::CreateTextWindow( int iTextToShow )
{
	...
	CMenuPanel *pMOTDPanel = CMessageWindowPanel_Create( m_iMOTDServerMode,  cText, cTitle, g_iPlayerClass == PC_UNDEFINED, false, 0, 0, ScreenWidth, ScreenHeight );
