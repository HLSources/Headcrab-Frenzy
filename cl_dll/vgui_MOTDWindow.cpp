...
#define MOTD_WINDOW_Y				YRES(40)
#define MOTD_WINDOW_SIZE_X			XRES(424)
#define MOTD_WINDOW_SIZE_Y			YRES(392)
...
{
public:
	CMessageWindowPanel( int server, const char *szMOTD, const char *szTitle, int iShadeFullScreen, int iRemoveMe, int x, int y, int wide, int tall );
...
//-----------------------------------------------------------------------------
CMenuPanel *CMessageWindowPanel_Create( int server, const char *szMOTD, const char *szTitle, int iShadeFullscreen, int iRemoveMe, int x, int y, int wide, int tall )
{
	return new CMessageWindowPanel( server, szMOTD, szTitle, iShadeFullscreen, iRemoveMe, x, y, wide, tall );
}

//-----------------------------------------------------------------------------
// Purpose: Constructs a message panel
//-----------------------------------------------------------------------------
CMessageWindowPanel::CMessageWindowPanel( int server, const char *szMOTD, const char *szTitle, int iShadeFullscreen, int iRemoveMe, int x, int y, int wide, int tall ) : CMenuPanel( iShadeFullscreen ? 100 : 255, iRemoveMe, x, y, wide, tall )
{
	...
	pText->setFgColor( 255, 255, 255, 0 );
	...
	if (!server)
	{
		CommandButton *pButton = new CommandButton("OK", iXPos + XRES(16), iYPos + iYSize - YRES(16) - BUTTON_SIZE_Y, CMENU_SIZE_X, BUTTON_SIZE_Y);
		pButton->addActionSignal(new CMenuHandler_TextWindow(HIDE_TEXTWINDOW));
		pButton->setParent(this);
	}
	else
	{
		CommandButton *pButton = new CommandButton("Ladder", iXPos + XRES(16), iYPos + iYSize - YRES(16) - BUTTON_SIZE_Y, CMENU_SIZE_X, BUTTON_SIZE_Y);
		pButton->addActionSignal(new CMenuHandler_TextWindow(HIDE_TEXTWINDOW_HCFSTART));
		pButton->setParent(this);

		pButton = new CommandButton("Random", iXPos + XRES(32) + CMENU_SIZE_X, iYPos + iYSize - YRES(16) - BUTTON_SIZE_Y, CMENU_SIZE_X, BUTTON_SIZE_Y);
		pButton->addActionSignal(new CMenuHandler_TextWindow(HIDE_TEXTWINDOW_HCFSTART2));
		pButton->setParent(this);
	}
