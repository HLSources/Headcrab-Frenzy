...
void V_GetMapChasePosition(int target, float * cl_angles, float * origin, float * angles)
{
	...
	VectorMA(origin, -512, forward, origin); 
}
...
void V_CalcSpectatorRefdef ( struct ref_params_s * pparams )
{
		...
		&& ent && g_iUser2 )
	{
		...
		//add renderfx
		gunModel->curstate.renderfx = ent->curstate.renderfx;
		gunModel->curstate.rendercolor.r = ent->curstate.rendercolor.r;
		gunModel->curstate.rendercolor.g = ent->curstate.rendercolor.g;
		gunModel->curstate.rendercolor.b = ent->curstate.rendercolor.b;
