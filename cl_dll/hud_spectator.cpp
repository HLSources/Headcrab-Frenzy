...
void CHudSpectator::DrawOverviewLayer()
{
	...
	float fZoom = m_OverviewData.zoom;
	

	// rotated view ?
	if ( m_OverviewData.rotated )
	{
		xStep = (2*4096.0f / fZoom ) / xTiles;
		yStep = -(2*4096.0f / (fZoom* screenaspect) ) / yTiles;

		y = ys + (4096.0f / (fZoom * screenaspect));
	...
	else
	{
		...
		x = xs + (4096.0f / (fZoom * screenaspect ));
...
void CHudSpectator::DrawOverviewEntities()
{
	...
	float			x,y,z, r,g,b, sizeScale = 2.0f;
	cl_entity_t *	ent;
	...
	VectorScale (forward, 256.0f, forward);
