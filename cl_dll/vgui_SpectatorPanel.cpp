...
#define OPTIONS_BUTTON_X	93
#define POLLS_BUTTON_X		92
...
void SpectatorPanel::ActionSignal(int cmd)
{
	switch (cmd)
	{
		...
		case SPECTATOR_PANEL_CMD_POLLS :		gViewPort->ShowCommandMenu( gViewPort->m_SpectatorPollsMenu );
												break;
...
void SpectatorPanel::Initialize()
{
	...
	m_PollsButton = new ColorButton( CHudTextMessage::BufferedLocaliseTextString( "#SPECT_POLLS" ), XRES(30 + OPTIONS_BUTTON_X), YRES(6), XRES(POLLS_BUTTON_X), YRES(20), false, false );
	m_PollsButton->setParent( m_BottomBorder );
	m_PollsButton->setContentAlignment( vgui::Label::a_center );
	m_PollsButton->setBoundKey( (char)255 );	// special no bound to avoid leading spaces in name 
	m_PollsButton->addActionSignal( new CSpectatorHandler_Command(this,SPECTATOR_PANEL_CMD_POLLS) );
	m_PollsButton->setUnArmedBorderColor ( 59, 58, 34, 48 );
	m_PollsButton->setArmedBorderColor ( 194, 202, 54, 0 );
	m_PollsButton->setUnArmedColor ( 143, 143, 54, 0 );
	m_PollsButton->setArmedColor ( 194, 202, 54, 0 );
	...
	m_PrevPlayerButton= new ColorButton("<", XRES( 15 + OPTIONS_BUTTON_X + POLLS_BUTTON_X + 30 ), YRES(6), XRES(24), YRES(20), false, false );
	...
	float flLabelSize = ( (ScreenWidth - (XRES ( CAMOPTIONS_BUTTON_X ) + 15)) - XRES ( 24 + 15 ) ) - XRES( (15 + OPTIONS_BUTTON_X + POLLS_BUTTON_X + 30) + 38 );

	m_BottomMainLabel = new Label( "Spectator Bottom", XRES( ( 15 + OPTIONS_BUTTON_X + POLLS_BUTTON_X + 30 ) + 31 ), YRES(6), flLabelSize, YRES(20) );
...
//	m_HideButton->setVisible(false);
	...
	m_PollsButton->setVisible(false);
...
//	m_HideButton->setVisible(isVisible);	m_HideButton->setArmed( false );
	...
	m_PollsButton->setVisible(isVisible);		m_PollsButton->setArmed( false );
	...
	if ( !isVisible )
	{
		...
		m_BottomMainLabel->setPos( XRES( ( 15 + OPTIONS_BUTTON_X + POLLS_BUTTON_X + 30 ) + 31 ), YRES(6) );
