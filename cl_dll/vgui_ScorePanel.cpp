...
SBColumnInfo g_ColumnInfo[NUM_COLUMNS] =
{
	...
	{"#SCORE",		40,			Label::a_center},
	{"#FRAGS",		40,			Label::a_center},
	{"#DEATHS",		46,			Label::a_center},
	{"#LATENCY",	46,			Label::a_center},
...
void ScorePanel::SortPlayers( int iTeam, char *team )
{
	...
	bool bHeadcrabTeamAdded = false;

	if ( iTeam != TEAM_SPECTATORS )
	{
		if (g_TeamInfo[TEAM_YES].score < g_TeamInfo[TEAM_HEADCRABS].score)
		{
			AddHeadcrabTeam();
			bHeadcrabTeamAdded = true;
		}
	}

	if ( (iTeam != TEAM_SPECTATORS) && !g_TeamInfo[TEAM_YES].scores_overriden )
	{
		g_TeamInfo[TEAM_YES].score = 0;
		g_TeamInfo[TEAM_YES].frags = 0;
		g_TeamInfo[TEAM_YES].deaths = 0;
		g_TeamInfo[TEAM_YES].players = 0;
	}
	...
	while ( 1 )
	{
		...
		int highest_frags = -9999999;
		int lowest_deaths = 9999999;
		int best_player;
		best_player = 0;
		...
		for ( int i = 1; i < MAX_PLAYERS; i++ )
		{
			if ( m_bHasBeenSorted[i] == false && g_PlayerInfoList[i].name && g_PlayerExtraInfo[i].score >= highest_score )
			{
				...
				if ((iTeam == TEAM_SPECTATORS) && !g_IsSpectator[i])
					continue;
				...
				if ( ent && !(team && stricmp(g_PlayerExtraInfo[i].teamname, team)) )  
				{
					...
					if ( pl_info->score > highest_score || pl_info->frags > highest_frags || pl_info->deaths < lowest_deaths )
					{
						best_player = i;
	...
	for ( int i = 1; i < MAX_PLAYERS; i++ )
	{
		...
		if ( ent && !(team && stricmp(g_PlayerExtraInfo[i].teamname, team)) )  
		{
			...
			if ( (iTeam != TEAM_SPECTATORS) && !g_TeamInfo[TEAM_YES].scores_overriden )
			{
				g_TeamInfo[TEAM_YES].score += pl_info->score;
				g_TeamInfo[TEAM_YES].frags += pl_info->frags;
				g_TeamInfo[TEAM_YES].deaths += pl_info->deaths;
				g_TeamInfo[TEAM_YES].players++;
			}
		}
	}
	...
	if ( iTeam != TEAM_SPECTATORS )
	{
		if (!bHeadcrabTeamAdded && (g_TeamInfo[TEAM_YES].score >= g_TeamInfo[TEAM_HEADCRABS].score))
			AddHeadcrabTeam();
	}
}

void ScorePanel::AddHeadcrabTeam()
{
	m_iIsATeam[ m_iRows ] = TEAM_HEADCRABS;
	m_iRows++;
...
void ScorePanel::FillGrid()
{
	...
	for(int row=0; row < NUM_ROWS; row++)
	{
		...
		for(int col=0; col < NUM_COLUMNS; col++)
		{
			...
			{
				// Get the team's data
				team_info = &g_TeamInfo[ TEAM_YES ];

				// team color text for team names
				pLabel->setFgColor(	100, 100, 255, 0 );
			...
			else if ( m_iIsATeam[row] == TEAM_HEADCRABS )
			{
				// Get the team's data
				team_info = &g_TeamInfo[ TEAM_HEADCRABS ];
				
				// grey text for spectators
				pLabel->setFgColor(255, 100, 100, 0);
				...
				pGridRow->SetRowUnderline(0, true, YRES(3), 255, 100, 100, 0);
			}
			else
			{
				// team color text for player names
				pLabel->setFgColor(	255,
									255,
									255,
									0 );
			...
			if ( m_iIsATeam[row] )
			{
				...
				case COLUMN_NAME:
					...
					{
						sprintf( sz2, CHudTextMessage::BufferedLocaliseTextString( "#Headcrabs" ) );
					}
					else
					{
						sprintf( sz2, CHudTextMessage::BufferedLocaliseTextString( gViewPort->GetTeamName(team_info->teamnumber) ) );
					}
				...
				case COLUMN_CLASS:
					break;
				case COLUMN_SCORE:
					if ( m_iIsATeam[row] == TEAM_YES || m_iIsATeam[row] == TEAM_HEADCRABS )
