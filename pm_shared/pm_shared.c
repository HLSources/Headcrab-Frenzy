...
int PM_CheckStuck (void)
{
	...
	// CSC: unstick player if it has got stuck in other physent (but more slightly)
	// CSC: this may happen with XPhysics ents
	// CSC: allow movement keys to fix stuck and not an attack key
	if ( pmove->cmd.buttons & ( IN_JUMP | IN_DUCK | IN_FORWARD | IN_BACK | IN_MOVELEFT | IN_MOVERIGHT) )
	{
		float x, y, z;
		float xystep;
		float zstep;
		...
		if ( pmove->physents[ hitent ].player != 0 )
		{
			xystep = 8.0;
			xyminmax = xystep;
			zstep = 16.0;
