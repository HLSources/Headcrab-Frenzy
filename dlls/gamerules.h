...
// Functions to verify the single/multiplayer status of a game
	...
	virtual const char *GetGameDescription( void ) { return "Headcrab Frenzy!"; }  // this is the game name that gets seen in the server browser
...
// Client spawn/respawn control
	...
	virtual edict_t *GetPlayerSpawnSpot( CBasePlayer *pPlayer, bool bFirstSpawn );// Place this player on their spawnspot and face them the proper direction.
...
// Powerup spawn/respawn control
	virtual BOOL FAllowPowerups( void ) = 0;//are powerups allowed
	virtual int PowerupShouldRespawn( CBaseEntity *pItem ) = 0;// Should this Powerup respawn?
	virtual float FlPowerupRespawnTime( CBaseEntity *pItem ) = 0;// when may this Powerup respawn?
	virtual Vector VecPowerupRespawnSpot( CBaseEntity *pItem ) = 0;// where in the world should this Powerup respawn?
...
// Monsters
	...
	virtual void HeadcrabBite( CBaseEntity *pHeadcrab, CBasePlayer *pVictim ) = 0;// Called each time a headcrab bites a player
	virtual void HeadcrabDied( CBaseEntity *pHeadcrab ) = 0;

// Immediately end a multiplayer game
	virtual void EndMultiplayerGame( void ) {}

// Polls
	virtual void CreatePoll( int playerIndex, int pollId ) {}
	virtual void VotePoll( int playerIndex, int voteId ) {}
	virtual int  GetActivePoll( void ) { return 0; }

	virtual void HCFRestart ( void ) = 0;

protected:
	void SaveHighScore( void );
	void LoadHighScore( void );

	HCFHighScore_t m_HighScoreLadder[3];
	HCFHighScore_t m_HighScoreRandom[3];
};
...
// Powerup spawn/respawn control
	virtual BOOL FAllowPowerups( void );
	virtual int PowerupShouldRespawn( CBaseEntity *pItem );
	virtual float FlPowerupRespawnTime( CBaseEntity *pItem );
	virtual Vector VecPowerupRespawnSpot( CBaseEntity *pItem );
...
// Teamplay stuff	
	...
	virtual void HeadcrabBite( CBaseEntity *pHeadcrab, CBasePlayer *pVictim ) {}
	virtual void HeadcrabDied( CBaseEntity *pHeadcrab ) {}

	virtual void HCFRestart ( void );
...
// Client spawn/respawn control
	...
	virtual BOOL FPlayerCanRespawn( CBasePlayer *pPlayer );
	virtual float FlPlayerSpawnTime( CBasePlayer *pPlayer );
	virtual edict_t *GetPlayerSpawnSpot( CBasePlayer *pPlayer, bool bFirstSpawn );
...
// Polls
	virtual void CreatePoll( int playerIndex, int pollId );
	virtual void VotePoll( int playerIndex, int voteId );
	virtual int  GetActivePoll( void ) { return m_iActivePoll; }
...
protected:
	...
	int m_HeadcrabScore;
	int m_HeadcrabFrags;
	int m_HeadcrabDeaths;
	int	m_iActivePoll;
	int m_iActivePollCreator;
	int m_iPollVote[MAX_CLIENTS];
};
