...
// still realize that they are teammates. (overridden for monsters that form groups)
	...
	virtual	float	GetGibScale( void ) { return 1.0f; }
...
#define PARALYZE_DURATION	3.0		// number of 2 second intervals to take damage
#define PARALYZE_DAMAGE		1.0		// damage to take each 2 second interval

#define NERVEGAS_DURATION	2
#define NERVEGAS_DAMAGE		5.0

#define POISON_DURATION		3.0
...
class CHCFManager : public CBaseEntity
{
public:
	void Spawn( void );
	void Precache( void );
	void Think( void );
	void StartGame( int iMode );
	void HeadcrabKilled( entvars_t *pevKiller, int iCombo );
	void SoundNotice( edict_t *pSource, const char *szSample );
	BOOL GameStarted( void ) { return m_fGameStarted; }
	float GameStartTime( void ) { return m_flStartTime; }
	BOOL CheatsUsed( void ) { return m_bCheated; }
...
private:
	int		m_iGameMode;
	BOOL	m_bCheated;
	float	m_flNextCheatMsg;
	BOOL	m_fGameStarted;
	BOOL	m_fPrepareSound;
	BOOL	m_bFirstBlood;
	float	m_flStartTime;
	float	m_flCurrentTime;
	int		m_iStartMsg;
	int		m_iMaxChildren;
	float	m_flNextMaxChildrenUpgrade;
	float	m_flNextHeadcrabUpgrade;
	float	m_flNextHeadcrabTypeUpgrade;
	float	m_flNextWeaponUpgrade;
	char	m_szSoundNotice[256];
	edict_t *m_pSoundSource;
	float	m_flSoundNoticeTime;
	float	m_flWaitForPlayersTime;
	int		m_iClientWaitForPlayersTime;
};