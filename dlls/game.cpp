...
cvar_t	hcf_mode	= {"hcf_mode","0", FCVAR_SERVER };
cvar_t	hcf_allowpowerups={"hcf_allowpowerups","1", FCVAR_SERVER };
cvar_t	hcf_waittime= {"hcf_waittime","20", FCVAR_SERVER };
cvar_t	hcf_public	= {"hcf_public","0", FCVAR_SERVER };
...
void Cmd_HCFRestart_f( void )
{
	if (g_pGameRules)
		g_pGameRules->HCFRestart();
}
...
{
	g_engfuncs.pfnAddServerCommand("hcf_restart", Cmd_HCFRestart_f);
	g_engfuncs.pfnAddServerCommand("hcf_validateStat", SV_ValidateStatServer);
	...
	CVAR_REGISTER (&hcf_mode);
	CVAR_REGISTER (&hcf_allowpowerups);
	CVAR_REGISTER (&hcf_waittime);
	CVAR_REGISTER (&hcf_public);
