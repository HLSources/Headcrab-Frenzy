...
//=========================================================
//=========================================================
edict_t *CGameRules :: GetPlayerSpawnSpot( CBasePlayer *pPlayer, bool bFirstSpawn )
{
	edict_t *pentSpawnSpot = EntSelectSpawnPoint( pPlayer, bFirstSpawn );
	...
	while ((pEntity = UTIL_FindEntityInSphere( pEntity, pPlayer->pev->origin, 32.0f )) != NULL)
	{
		if ( (pEntity->pev->takedamage != DAMAGE_NO) && ( pEntity != pPlayer ))
		{
			pEntity->TakeDamage ( pPlayer->pev, pPlayer->pev, 9999.0f, DMG_ALWAYSGIB | DMG_DROWNRECOVER );
		}
	}
...
void CGameRules::LoadHighScore ( void )
{
	byte    *aMemFile;
	byte    *pMemFile;
	...
	memset(m_HighScoreLadder, 0, sizeof(m_HighScoreLadder[0])*3);
	memset(m_HighScoreRandom, 0, sizeof(m_HighScoreRandom[0])*3);

	strcpy ( szFilename, "/score/" );
	strcat ( szFilename, (char *)STRING( gpGlobals->mapname ) );
	...
	if (length != (sizeof(m_HighScoreLadder[0])+sizeof(m_HighScoreRandom[0]))*3+4 )
	{
		ALERT(at_console, "LoadHighScore: bad score file \"%s\"!\n", szFilename);
		FREE_FILE(aMemFile);
		return;
	}

	unsigned int magic = *(unsigned int*)(pMemFile);
	pMemFile += 4;

	if (magic != 0xDEADBABE)
	{
		ALERT(at_console, "LoadHighScore: bad score file!\n");
		FREE_FILE(aMemFile);
		return;
	}
	...
	for (i = 0; i < 3; i++)
	{
		memcpy(&m_HighScoreLadder[i], pMemFile, sizeof(m_HighScoreLadder[i]));
		pMemFile += sizeof(m_HighScoreLadder[i]);

		memcpy(&m_HighScoreRandom[i], pMemFile, sizeof(m_HighScoreRandom[i]));
		pMemFile += sizeof(m_HighScoreRandom[i]);
	}

	FREE_FILE(aMemFile);
}

void CGameRules::SaveHighScore ( void )
{
	char	szFilename[MAX_PATH];
	...
	ALERT(at_console, "Writing %s\n", szFilename);

	FILE *fp = fopen(szFilename, "wb");

	if (!fp)
	{
		ALERT(at_console, "SaveHighScore: cannot write file \"%s\"!\n", szFilename);
		return;
	}
	...
	for (i = 0; i < 3; i++)
	{
		fwrite(&m_HighScoreLadder[i], sizeof(m_HighScoreLadder[i]), 1, fp);
		fwrite(&m_HighScoreRandom[i], sizeof(m_HighScoreRandom[i]), 1, fp);
	}
	fclose(fp);
}
