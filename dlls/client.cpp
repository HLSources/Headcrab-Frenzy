...
void respawn(entvars_t* pev, BOOL fCopyCorpse)
{
	...
	if (pManager && pManager->GameStarted() && (pManager->GetGameMode() > 0) && !(pev->weapons & (1<<WEAPON_CROWBAR)))
		GetClassPtr( (CBasePlayer *)pev)->GiveNamedItem( "weapon_crowbar" );
}
...
void ClientPutInServer( edict_t *pEntity )
{
	...
	if (bSpectate)
	{
		edict_t *pentSpawnSpot = g_pGameRules->GetPlayerSpawnSpot( pPlayer, true );
		pPlayer->StartObserver( pev->origin, VARS(pentSpawnSpot)->angles);
		UTIL_ShowMessage("HCOBSERVER", pPlayer);
	}
}
...
void ClientCommand( edict_t *pEntity )
{
	...
	else if ( FStrEq( pcmd, "spec_poll" )  )	// spectator poll
	{
		CBasePlayer * pPlayer = GetClassPtr((CBasePlayer *)pev);
		if ( pPlayer->IsObserver() )
			pPlayer->Observer_Poll(atoi( CMD_ARGV(1) ));
	}
	else if ( FStrEq( pcmd, "pollvote" )  )	// poll vote
	{
		CBasePlayer * pPlayer = GetClassPtr((CBasePlayer *)pev);
		pPlayer->Observer_PollVote(atoi( CMD_ARGV(1) ));
	}
	else
	{
...
// player pain sounds
	...
	//hzfrenzy
	PRECACHE_SOUND("hcfrenzy/doublekill.wav");
	PRECACHE_SOUND("hcfrenzy/triplekill.wav");
	PRECACHE_SOUND("hcfrenzy/megakill.wav");
	PRECACHE_SOUND("hcfrenzy/multikill.wav");
	PRECACHE_SOUND("hcfrenzy/ultrakill.wav");
	PRECACHE_SOUND("hcfrenzy/monsterkill.wav");
	PRECACHE_SOUND("hcfrenzy/godlike.wav");
	PRECACHE_SOUND("hcfrenzy/firstblood.wav");
	PRECACHE_SOUND("hcfrenzy/rampage.wav");
	PRECACHE_SOUND("hcfrenzy/unstoppable.wav");
	PRECACHE_SOUND("hcfrenzy/nicecatch.wav");
	PRECACHE_SOUND("hcfrenzy/takenlead.wav");
	PRECACHE_SOUND("hcfrenzy/prepare.wav");
	PRECACHE_SOUND("hcfrenzy/cd1.wav");
	PRECACHE_SOUND("hcfrenzy/cd2.wav");
	PRECACHE_SOUND("hcfrenzy/cd3.wav");
	PRECACHE_SOUND("hcfrenzy/damage.wav");
	PRECACHE_SOUND("hcfrenzy/lostmatch.wav");
			...
			while ( pPlayerItem )
			{
				...
				if ( gun && gun->UseDecrement() )
				{
					...
					if ( II.iId >= 0 && II.iId < 32 )
					{
						...
						item->fuser4					= gun->m_flUpgradeMod;
