...
extern edict_t *EntSelectSpawnPoint( CBaseEntity *pPlayer, bool bFirstSpawn );
...
TYPEDESCRIPTION	CBasePlayer::m_playerSaveData[] = 
{
	...
	DEFINE_FIELD( CBasePlayer, m_flLastSlashDamage, FIELD_TIME ),
	DEFINE_FIELD( CBasePlayer, m_flDamageIncrease, FIELD_TIME ),
	DEFINE_ARRAY( CBasePlayer, m_iSpecialKillStat, FIELD_INTEGER, 7 ),
	DEFINE_FIELD( CBasePlayer, m_flRemoveFrenzy, FIELD_TIME ),
	DEFINE_FIELD( CBasePlayer, m_flRemoveWeaponShell, FIELD_TIME ),
	DEFINE_FIELD( CBasePlayer, m_flFrenzyTime, FIELD_FLOAT ),
	DEFINE_FIELD( CBasePlayer, m_flParalyzeTime, FIELD_TIME ),
	DEFINE_FIELD( CBasePlayer, m_flNextHealthRegen, FIELD_TIME ),
	DEFINE_FIELD( CBasePlayer, m_iBonusLives, FIELD_INTEGER ),	
	DEFINE_FIELD( CBasePlayer, m_flFrenzy, FIELD_FLOAT ),	
	DEFINE_FIELD( CBasePlayer, m_bFrenzyMode, FIELD_BOOLEAN ),
	
	DEFINE_FIELD( CBasePlayer, m_pTank, FIELD_EHANDLE ),
	DEFINE_FIELD( CBasePlayer, m_iHideHUD, FIELD_INTEGER ),
	DEFINE_FIELD( CBasePlayer, m_iFOV, FIELD_INTEGER ),
	DEFINE_FIELD( CBasePlayer, m_bKillRampage, FIELD_BOOLEAN ),
	DEFINE_FIELD( CBasePlayer, m_iLastSpawnKills, FIELD_INTEGER ),
	DEFINE_FIELD( CBasePlayer, m_iMaxLastSpawnKills, FIELD_INTEGER ),
	DEFINE_FIELD( CBasePlayer, m_iNextExtraLifeFrags, FIELD_INTEGER ),
	DEFINE_FIELD( CBasePlayer, m_flUpgradeMod, FIELD_FLOAT ),

	DEFINE_ARRAY( CBasePlayer, m_flPowerupTimes, FIELD_INTEGER, POWERUP_MAX_COUNT ),
...
void LinkUserMessages( void )
{
	...
	gmsgScoreInfo = REG_USER_MSG( "ScoreInfo", 13 );
	gmsgTeamInfo = REG_USER_MSG( "TeamInfo", -1 );  // sets the name of a player's team
	gmsgTeamScore = REG_USER_MSG( "TeamScore", 9 );  // sets the score of a team on the scoreboard (0=folks, 1=headcrabs)
	...
	gmsgFrenzy = REG_USER_MSG( "Frenzy", 1 );
	gmsgPowerup = REG_USER_MSG( "Powerup", 2 );
	gmsgEndGame = REG_USER_MSG( "EndGame", 1 );
	gmsgRestartGame = REG_USER_MSG( "RestartGame", 1 );
	gmsgPoll = REG_USER_MSG( "Poll", -1 );
...
int CBasePlayer :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	...
	if (m_flPowerupTimes[POWERUP_INVUL] > 0.1f)
		return 0;
	...
	if (bitsDamageType & DMG_SLASH)
		m_flLastSlashDamage = gpGlobals->time;
...
// choose to respawn.
/*	if (( gpGlobals->time > (m_fDeadTime + 2) ) && !(m_afPhysicsFlags & PFLAG_OBSERVER) )
	{
		// go to dead camera. 
		StartDeathCam( false );
	}*/
...
// wait for any button down,  or mp_forcerespawn is set and the respawn time is up
	if (!fAnyButtonDown 
		&& !( forcerespawn.value > 0 && (gpGlobals->time > (m_fDeadTime + 5))) )
		return;
...
//=========================================================
void CBasePlayer::StartDeathCam( bool bCopyCorpse )
{
		...
		if ( !FNullEnt( tr.pHit ) )
		{
			...
			if (pEntity->Classify() == CLASS_PLAYER )
			{
				...
				if ( g_pGameRules->PlayerRelationship( this, pEntity ) == GR_TEAMMATE )
				{
					...
					newSBarState[ SBAR_ID_TARGETARMOR ] = min(100, pEntity->pev->armorvalue); //No need to get it % based since 100 it's the max.
				}
...
extern Vector g_vecPowerupColors[POWERUP_MAX_COUNT];

void CBasePlayer :: RestoreRenderEffects( void )
{
	if (m_bFrenzyMode)
		return;
	...
	{
		if (m_flPowerupTimes[i] > 0.1f)
		{
			//active
			pev->renderfx = kRenderFxGlowShell;
			pev->rendercolor = g_vecPowerupColors[i];
			return;
		}
	}
...
void CBasePlayer::PreThink(void)
{
	...
	if (m_flRemoveFrenzy > 0.1f)
	{
		if (m_flRemoveFrenzy < gpGlobals->time)
		{
			m_flRemoveFrenzy = 0;
			m_bFrenzyMode = FALSE;
			m_flFrenzy = 0;
			RestoreRenderEffects();
		}
		else
		{
			m_flFrenzy = 99 * ((m_flRemoveFrenzy - gpGlobals->time) / m_flFrenzyTime);
		}
	}

	if (m_flRemoveWeaponShell > 0.1f)
	{
		if (m_flRemoveWeaponShell < gpGlobals->time)
		{
			RestoreRenderEffects();
			m_flRemoveWeaponShell = 0;
		}
	}
	...
	//check rampage
	if (m_iLastSpawnKills > HCF_UNSTOPPABLE_FRAGS)
	{
		if (m_bKillRampage < 2)
		{
			m_bKillRampage = 2;
			EMIT_SOUND(ENT(pev), CHAN_VOICE, "hcfrenzy/unstoppable.wav", 1.0f, ATTN_NORM);
			UTIL_ScreenFade( this, Vector(255,100,255), 4.0f, 2.0f, 75, FFADE_IN );
			pev->gamestate += HCF_UNSTOPPABLE_SCORE;
		}
	}
	if (m_iLastSpawnKills > HCF_RAMPAGE_FRAGS)
	{
		if (m_bKillRampage < 1)
		{
			m_bKillRampage = 1;
			EMIT_SOUND(ENT(pev), CHAN_VOICE, "hcfrenzy/rampage.wav", 1.0f, ATTN_NORM);
			UTIL_ScreenFade( this, Vector(255,100,100), 2.0f, 1.0f, 55, FFADE_IN );
			pev->gamestate += HCF_RAMPAGE_SCORE;
		}
	}

	//extra life
	if ( IsAlive() && pev->frags >= m_iNextExtraLifeFrags)
	{
		if (pev->armorvalue < 999)
		{
			UTIL_ShowMessageAll("HCEXTRALIFE");
			pev->armorvalue++;
			m_iBonusLives++;
			UTIL_ScreenFade( this, Vector(100,255,100), 2.0f, 0.5f, 55, FFADE_IN );
		}
		m_iNextExtraLifeFrags += HCF_EXTRALIFE_FRAGS;
	}
...
void CBasePlayer::UpdatePowerups() 
{
	...
	//damage mod
	if (m_flDamageIncrease > 0.1f && m_flDamageIncrease < gpGlobals->time)
	{
		m_flDamageIncrease = 0.0f;
	}

	//regen
	if (IsAlive() && ((int)floor(pev->health) > 0) && (pev->health < pev->max_health) && (m_flNextHealthRegen < gpGlobals->time))
	{
		pev->health++;
		...
		//check if fast regen
		if (m_flPowerupTimes[POWERUP_REGEN] > 0.1f)
			m_flNextHealthRegen = gpGlobals->time + (1.0f / (HCF_LIFE_REGEN * 3.0f));
		else
			m_flNextHealthRegen = gpGlobals->time + (1.0f / HCF_LIFE_REGEN);
	}	
}
...
void CBasePlayer::CheckTimeBasedDamage() 
{
	...
	if ( m_bitsDamageType & DMG_PARALYZE )
	{
		g_engfuncs.pfnSetClientMaxspeed( ENT(pev), HCF_PARALYZE_SPEED + (g_psv_maxspeed->value - HCF_PARALYZE_SPEED) * ((gpGlobals->time - m_flParalyzeTime) / PARALYZE_DURATION) );
	}
	else
	{
		g_engfuncs.pfnSetClientMaxspeed( ENT(pev), 0 );
	}
...
//				TakeDamage(pev, pev, SLOWFREEZE_DAMAGE, DMG_GENERIC);
			...
			else
			{
				...
				if (i == itbd_Paralyze)
					m_flParalyzeTime = gpGlobals->time;
			}
		}
	}
}
...
============
*/
edict_t *EntSelectSpawnPoint( CBaseEntity *pPlayer, bool bFirstSpawn )
{
	CBaseEntity *pSpot;
	edict_t		*player;
...
// choose a info_player_deathmatch point
	...
	else if ( !bFirstSpawn || g_pGameRules->IsDeathmatch() )
	{
		pSpot = g_pLastSpawn;
...
void CBasePlayer::Spawn( void )
{
	...
	m_flLastSlashDamage = gpGlobals->time;
	m_flDamageIncrease = 0;
	m_bKillRampage = 0;

	if (m_iLastSpawnKills > m_iMaxLastSpawnKills)
		m_iMaxLastSpawnKills = m_iLastSpawnKills;
	m_iLastSpawnKills = 0;
	...
	memset(m_flPowerupTimes, 0, sizeof(m_flPowerupTimes));

	for (i = 0; i < POWERUP_MAX_COUNT; i++)
		m_iClientPowerupTimes[i] = -1;
...
// dont let uninitialized value here hurt the player
	...
	g_pGameRules->GetPlayerSpawnSpot( this, bFirstSpawn );
...
void CBasePlayer :: UpdateClientData( void )
{
	...
	if (m_flFrenzy != m_iClientFrenzy)
	{
		int iFrenzy = max( m_flFrenzy, 0 );  // make sure that no negative health values are sent

		// send "health" update message
		MESSAGE_BEGIN( MSG_ONE, gmsgFrenzy, NULL, pev );
			WRITE_BYTE( iFrenzy );
		MESSAGE_END();

		m_iClientFrenzy = m_flFrenzy;
	}
	...
	for (i = 0; i < POWERUP_MAX_COUNT; i++)
	{
		...
		int curVal = HCF_POWERUP_ACTIVE_TIME - (int)floor((gpGlobals->time - m_flPowerupTimes[i]));

		if (curVal <= 0 || !IsAlive())
		{
			m_flPowerupTimes[i] = 0;
			curVal = 0;
			bRestorePowerupFx = true;
		}

		if (m_iClientPowerupTimes[i] != curVal)
		{
			m_iClientPowerupTimes[i] = curVal;

			MESSAGE_BEGIN( MSG_ONE, gmsgPowerup, NULL, pev );
				WRITE_BYTE( i );
				WRITE_BYTE( 255 * min( (float)curVal / HCF_POWERUP_ACTIVE_TIME, 1.0f ) );
			MESSAGE_END();
		}		
	}

	if (bRestorePowerupFx)
		RestoreRenderEffects();
...
void CBasePlayer :: ActivateFrenzy( float flMod )
{
	if (m_bFrenzyMode || m_flFrenzy < 100)
		return;
	...
	m_flFrenzyTime = HCF_FRENZY_TIME * (1.0f + flMod * 2.0f);
	m_flRemoveFrenzy = gpGlobals->time + m_flFrenzyTime;

	EMIT_SOUND( ENT(pev), CHAN_AUTO, "hcfrenzy/damage.wav", 1.0, ATTN_NORM);
}

void CBasePlayer :: ActivateWeaponShell( void )
{
	if (m_bFrenzyMode)
		return;
	...
	m_flRemoveWeaponShell = gpGlobals->time + 3.0f;
}
...
void CBasePlayer::Observer_Poll( int pollId )
{
	if (!g_pGameRules)
		return;

	if (pev->flags & FL_ZOMBIE)
	{
		g_engfuncs.pfnServerPrint( UTIL_VarArgs("%s: zombie can't create poll.\n", STRING(pev->netname)) );
		return;
	}

	if (pev->flags & FL_SPECTATOR)
	{
		g_engfuncs.pfnServerPrint( UTIL_VarArgs("%s: pure spectator can't create poll.\n", STRING(pev->netname)) );
		return;
	}

	if (g_pGameRules->GetActivePoll() != 0)
	{
		g_engfuncs.pfnServerPrint( UTIL_VarArgs("%s: poll already active.\n", STRING(pev->netname)) );
		return;
	}

	if (m_flNextPollTime > gpGlobals->time)
	{
		//prevent poll spam
		ClientPrint(pev, HUD_PRINTCENTER, UTIL_VarArgs("Wait for %d secs before starting poll", (int)ceil(m_flNextPollTime-gpGlobals->time) ));
		return;
	}

	m_flNextPollTime = gpGlobals->time + 10.0f;

	g_pGameRules->CreatePoll( entindex(), pollId );
}

void CBasePlayer::Observer_PollVote( int voteId )
{
	if (!g_pGameRules)
		return;
	...
	if (pev->flags & FL_ZOMBIE)
	{
		g_engfuncs.pfnServerPrint( UTIL_VarArgs("%s: zombie can't vote on poll.\n", STRING(pev->netname)) );
		return;
	}

	if (pev->flags & FL_SPECTATOR)
	{
		g_engfuncs.pfnServerPrint( UTIL_VarArgs("%s: pure spectator can't vote on poll.\n", STRING(pev->netname)) );
		return;
	}

	g_pGameRules->VotePoll( entindex(), voteId );
}

BOOL CBasePlayer::AnyPowerupActive( void )
{
	for (int i = 0; i < POWERUP_MAX_COUNT; i++)
	{
		if (m_flPowerupTimes[i] > 0.1f)
...
BOOL CBasePlayer::IsInvisible( void )
{
	if (pev->flags & (FL_SPECTATOR|FL_ZOMBIE))
		return TRUE;

	return (m_flPowerupTimes[POWERUP_INVIS] > 0.1f) ? TRUE : FALSE; 
}
